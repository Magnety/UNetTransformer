...
class UNet(nn.Module):
    def __init__(self, in_dim, out_dim, num_filters):
        ...
        activation = nn.LeakyPReLU(0.2, inplace=True)
        #1,1,32,384,384
        # Down sampling
...
class AttU_Net(nn.Module):
    ...
    def __init__(self, img_ch=1, output_ch=1):
        ...
        self.extra_feature1 = nn.Sequential(nn.AdaptiveMaxPool3d((1, 1, 1)), nn.PReLU())
        self.extra_feature2 = nn.Sequential(nn.AdaptiveMaxPool3d((1, 1, 1)), nn.PReLU())
        self.extra_feature3 = nn.Sequential(nn.AdaptiveMaxPool3d((1, 1, 1)), nn.PReLU())
        self.extra_feature4 = nn.Sequential(nn.AdaptiveMaxPool3d((1, 1, 1)), nn.PReLU())
        self.extra_feature5 = nn.Sequential(nn.AdaptiveMaxPool3d((1, 1, 1)), nn.PReLU())
        self.extra_feature6 = nn.Sequential(nn.AdaptiveMaxPool3d((1, 1, 1)), nn.PReLU())
        self.extra_feature7 = nn.Sequential(nn.AdaptiveMaxPool3d((1, 1, 1)), nn.PReLU())
        self.extra_feature8 = nn.Sequential(nn.AdaptiveMaxPool3d((1, 1, 1)), nn.PReLU())
        self.fc1 = nn.Sequential(nn.Linear(688, 256), nn.PReLU())
...
class R2AttU_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        down2 = self.Maxpool1(down1)
        down2 = self.RRCNN2(down2)

        e3 = self.Maxpool2(down2)
        e3 = self.RRCNN3(e3)
        ...
        d3 = self.Up3(d4)
        down2 = self.Att3(g=d3, x=down2)
        d3 = torch.cat((down2, d3), dim=1)
        d3 = self.Up_RRCNN3(d3)

        d2 = self.Up2(d3)
        down1 = self.Att2(g=d2, x=down1)
        d2 = torch.cat((down1, d2), dim=1)
