...
class VNet(nn.Module):
    ...
    def __init__(self, elu=True, nll=False):
        ...
        self.down_tr32 = DownTransition(16, 1, elu, 2)
        self.down_tr64 = DownTransition(32, 2, elu, 2)
        self.down_tr128 = DownTransition(64, 3, elu, 2, dropout=True)
        self.down_tr256 = DownTransition(128, 2, elu, 1, dropout=True)
        self.up_tr256 = UpTransition(256, 256, 2, elu, 1, dropout=True)
        self.up_tr128 = UpTransition(256, 128, 2, elu, 2, dropout=True)
        self.up_tr64 = UpTransition(128, 64, 1, elu, 2)
        self.up_tr32 = UpTransition(64, 32, 1, elu, 2)
    ...
    def forward(self, x):
        ...
        out256 = F.upsample(out256, size=out128.size()[2:], mode='trilinear')
        print("out256")
        print(out256.shape)
        print("out128")
        print(out128.shape)
        ...
        print(out.shape)
        #
        out = F.upsample(out, size=out64.size()[2:], mode='trilinear')

        out = self.up_tr128(out, out64)
        print("4444")
        print(out.shape)
        #
        ex_feature4 = self.ex_feature4(out)
        out = F.upsample(out, size=out32.size()[2:], mode='trilinear')

        seg_out = self.up_tr64(out, out32)
        seg_out= F.upsample(seg_out, size=out16.size()[2:], mode='trilinear')
        seg_out = self.up_tr32(seg_out, out16)

        seg_out = self.out_tr(seg_out)
